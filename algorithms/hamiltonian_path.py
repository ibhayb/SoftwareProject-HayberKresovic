# Finding a Hamiltonian path in a graph (with directed edges)
...
##     ]
### open loop in the opposite direction
##g = [[0, x, x, x, x, x, x, 1], # node S
##     [1, 0, x, x, x, x, x, x], # node T
##     [x, 1, 0, x, x, x, x, x], # node U
##     [x, x, 1, 0, x, x, x, x], # node V
...
# just try out all permutations of the nodes
def find_hamiltonian_path():
   if len(g) == 0:
      return None
   for path in itertools.permutations(range(len(g))):
       valid_path = True
       i = 0
       while valid_path and i < len(path)-1:
